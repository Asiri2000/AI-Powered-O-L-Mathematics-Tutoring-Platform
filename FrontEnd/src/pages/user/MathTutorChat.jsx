...
import { Bot, User, Send, Sparkles, Loader2, Calculator, Brain } from 'lucide-react';
import api from '../../api';
  ...
  const [messages, setMessages] = useState([
    {
      id: 1,
      sender: 'bot',
      text: "Hello! 👋 I'm your Mathematics Tutor, powered by AI. I can help you with:\n\n• Solving math problems step-by-step\n• Explaining concepts from the G.C.E. O/L syllabus\n• Quadratic equations, geometry, algebra & more\n\nWhat would you like to learn today?",
    },
  ]);
  ...
  const handleSend = async (text) => {
    ...
    try {
      ...
      const botMsg = {
        ...
        sender: 'bot',
        text: data.response,
        source: data.source, // 'deterministic' | 'model'
      };
        ...
        <div className="flex-1 overflow-y-auto mb-6 pr-2 space-y-4">
          {messages.map((msg) => (
            ...
            >
              ...
              <div className="max-w-[80%]">
                ...
                {/* Source indicator for deterministic responses */}
          ...
          {messages.length === 1 && !isLoading && (
            <div className="mt-6 animate-fade-in">
              <p className="text-gray-500 mb-3 ml-14 text-sm">Try asking me:</p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 ml-14">
                {quickQuestions.map((q, index) => (
                  <button
                    ...
                    disabled={isLoading}
                    className="text-left p-3.5 bg-[#f3f4f6] hover:bg-green-50 border border-transparent hover:border-green-200 rounded-xl text-gray-700 transition-all duration-200 text-sm font-medium disabled:opacity-50"
                  >
                    {q}
                  </button>
                ))}
